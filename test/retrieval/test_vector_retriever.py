...
def _chunks():
    ...
    return [Chunk(id=f"c{i}", text=f"Testo {i}", metadata=Metadata(source=s, title="S")) for i in range(3)]


def _retriever():
    chunks = _chunks()
    store = FaissVectorStore()
    # ogni chunk ha il suo vettore unitario ortogonale
    store.add(np.eye(3, dtype=np.float32))
    return FaissVectorRetriever(chunks, _FakeEmbedder(), store), chunks
